...
import { getPositions, addToLiquidationQueue, getFundingTracker } from '../stores/positions.js'
...
export async function processNewPrice(market, price) {
	...
	for (const position of positions) {
		...
		if (pnl <= -1 * threshold) {
			console.log('adding to liq queue', position.user, position.asset, market);
